...
class ModuleB:
    ...
    def create_widgets(self):
        ...
        self.grammar1_rb = ttk.Radiobutton(
            grammar_frame, 
            text="Gramática 1 transformada: E → TE', E' → +TE' | ε, T → FT', T' → *FT' | ε, F → (E) | id", 
        ...
        self.grammar2_rb = ttk.Radiobutton(
            grammar_frame, 
            text="Gramática 2 transformada: L → A | F, F → (S), S → LS', S' → LS' | ε, A → num | id", 
            variable=self.grammar_var, 
            value="grammar2",
    ...
    def setup_grammar1(self):
        ...
        self.console_callback("Gramática 1 transformada seleccionada: E → TE', E' → +TE' | ε, T → FT', T' → *FT' | ε, F → (E) | id")
    
    def setup_grammar2(self):
        self.current_grammar = "grammar2"
        self.grammar_var.set("grammar2")
        self.console_callback("Gramática 2 transformada seleccionada: L → A | F, F → (S), S → LS', S' → LS' | ε, A → num | id")
    ...
    def E(self):
        ...
        return self.T() and self.E_prime()
    ...
    def T(self):
        ...
        return self.F() and self.T_prime()
    ...
    def S(self):
        ...
        return self.L() and self.S_prime()
    
    def S_prime(self):
        self.console_callback(f"Llamando a S'() en token {self.tokens[self.current_token] if self.current_token < len(self.tokens) else 'EOF'}")
        if self.current_token < len(self.tokens) and (self.tokens[self.current_token] == 'num' or self.tokens[self.current_token] == 'id' or self.tokens[self.current_token] == '('):
            return self.L() and self.S_prime()
        return True  # ε producción
